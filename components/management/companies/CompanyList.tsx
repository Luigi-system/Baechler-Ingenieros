    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                 <div>
                    <h3 className="text-xl font-bold text-base-content">Gestionar Empresas</h3>
                        ...
                        <table className="w-full table-auto">
                            <thead className="bg-base-300 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-semibold text-neutral uppercase tracking-wider">Dirección</th>
                                    <th className="px-6 py-3 text-left text-xs font-semibold text-neutral uppercase tracking-wider">Distrito</th>
                                    <th className="px-6 py-3 text-left text-xs font-semibold text-neutral uppercase tracking-wider">RUC</th>
                                    <th className="px-6 py-3 text-left text-xs font-semibold text-neutral uppercase tracking-wider">Acciones</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-base-border">
                                {filteredCompanies.map(company => (
                                <tr key={company.id} className="hover:bg-base-300/50 even:bg-base-300/20 transition-colors">
                                    <td className="px-6 py-4 text-sm font-medium text-base-content break-words">{company.nombre}</td>
                                    <td className="px-6 py-4 text-sm text-neutral break-words">{company.direccion || 'N/A'}</td>
                                    <td className="px-6 py-4 text-sm text-neutral break-words">{company.distrito || 'N/A'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral">{company.ruc || 'N/A'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <button onClick={() => handleEdit(company)} className="text-primary hover:text-primary-focus p-1 rounded-full hover:bg-primary/10 transition"><EditIcon className="h-5 w-5"/></button>
                                        <button onClick={() => handleDelete(company.id)} className="text-error hover:text-error/80 p-1 rounded-full hover:bg-error/10 transition"><TrashIcon className="h-5 w-5"/></button>
                                    </td>
                                </tr>
                                ))}
                            </tbody>
