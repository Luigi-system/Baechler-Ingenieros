    ...
    <>
      ...
      <aside 
          className={`
              ...
              flex flex-col bg-base-200 shadow-xl transition-all duration-300 ease-in-out
              md:translate-x-0
              ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}
              w-64 ${isCollapsed ? 'md:w-20' : 'md:w-64'}
          `}
      >
        <div className={`flex border-b border-base-border transition-all duration-300 ${!showExpanded ? 'h-20 justify-center items-center' : 'min-h-20 px-4 py-4 items-start'}`}>
          <img src={logoUrl} alt="App Logo" className={`transition-all duration-300 object-contain h-10 w-10 shrink-0 ${!showExpanded ? '' : 'mt-1'} ${isLogoAnimated ? 'logo-jiggle-animation' : ''}`} />
          {showExpanded && (
            <div
