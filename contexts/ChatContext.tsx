...
import {
  directSupabaseSystemInstruction,
  responseSchema,
  executeQueryOnDatabase_Gemini,
  getAggregateData_Gemini,
  performAction_Gemini,
  callExternalAgentWithQuery_Gemini,
  callExternalAgentWithData_Gemini, // New Gemini agent tools
  executeQueryOnDatabase_OpenAI,
  getAggregateData_OpenAI,
  performAction_OpenAI,
  callExternalAgentWithQuery_OpenAI,
  callExternalAgentWithData_OpenAI, // New OpenAI agent tools
  handleFunctionExecution,
  agenteOrchestratorSystemInstruction,
...
const WELCOME_MESSAGE: Message = {
  ...
  content: {
    ...
    suggestions: [
      "Muéstrame los 5 reportes de servicio más recientes",
      "¿Cuántas empresas hay en total?",
      "Crea un gráfico de reportes por empresa",
    ],
  },
};
...
const mapAgentResponseToAIResponse = (rawAgentResponse: any): AIResponse => {
  ...
  if (
    rawAgentResponse &&
    typeof rawAgentResponse === "object" &&
    "error" in rawAgentResponse &&
    "contexto" in rawAgentResponse &&
    "data" in rawAgentResponse
  ) {
    ...
    const suggestions = [
      "Listar todos los registros de esta tabla",
      "¿Qué más puedo hacer?",
    ];
    if (Array.isArray(data) && data.length > 0 && typeof data[0] === "object") {
      const headers = Object.keys(data[0]);
      const rows = data.map((item: any) =>
        headers.map((header) => safeStringify(item[header]))
      );
      const table = { headers, rows };

      if (
        data.length === 1 &&
        (String(contexto).toLowerCase().includes("creado") ||
          String(contexto).toLowerCase().includes("actualizado"))
      ) {
  ...
  console.warn(
    "La respuesta del agente no sigue la estructura {error, contexto, data}. Usando lógica de mapeo de fallback."
  );
  if (
    rawAgentResponse &&
    typeof rawAgentResponse === "object" &&
    "displayText" in rawAgentResponse
  ) {
  ...
  if (rawAgentResponse === null || typeof rawAgentResponse !== "object") {
    return {
      displayText: `Recibí una respuesta inesperada del agente: ${String(
        rawAgentResponse
      )}.`,
      statusDisplay: {
        icon: "error",
        title: "Error Inesperado",
        message: "La respuesta del agente no es válida o está vacía.",
      },
    };
  }
  ...
  return {
    ...
    statusDisplay: {
      ...
      message: "Se recibió una respuesta en un formato no estándar.",
    },
  };
};
  ...
  useEffect(() => {
    ...
    if (service === "gemini" && geminiClient) {
      try {
          ...
          ? [
              {
                functionDeclarations: [
                  callExternalAgentWithQuery_Gemini,
                  callExternalAgentWithData_Gemini,
                ],
              },
            ]
          : [
        ...
        const currentSystemInstruction = isAgenteEnabled
          ? agenteOrchestratorSystemInstruction
          : directSupabaseSystemInstruction;

        const newChat = geminiClient.chats.create({
          model: "gemini-2.5-flash", // Using a powerful model for reasoning.
          config: {
            tools: geminiTools,
    ...
    async (prompt: string) => {
      ...
      try {
        ...
        const formSubmissionPattern =
          "El usuario ha completado el formulario con los siguientes datos: ";
        if (
          isAgenteEnabled &&
          agenteClient &&
          prompt.startsWith(formSubmissionPattern)
        ) {
          console.log(
            "Detectado envío de formulario. Procesando directamente con el agente externo..."
          );
          const jsonString = prompt.substring(
            formSubmissionPattern.length,
            prompt.indexOf(". Procede a crear el registro en la base de datos.")
          );
          let formData: any;
          try {
            ...
            throw new Error(
              "Formato JSON inválido en el envío del formulario: " + e.message
            );
          }
          ...
          const loadingMessageContent: AIResponse = {
            displayText:
              "El asistente está procesando los datos del formulario y guardando el registro en la base de datos...",
            statusDisplay: {
              icon: "info",
              title: "Procesando...",
              message: "Enviando datos al agente externo...",
            },
          };
          setMessages((prev) => [
            ...prev,
          ...
          for (let attempt = 0; attempt <= MAX_AGENT_RETRIES; attempt++) {
            try {
              rawAgentResponse = await agenteClient.sendToAgent(
                { data: formData },
                agenteWebhookUrl
              );
              ...
              if (
                ...
                throw new Error(
                  "Respuesta del agente externo no es un objeto JSON válido."
                );
              }
              ...
              console.error(
                `Intento ${attempt + 1}/${
                  MAX_AGENT_RETRIES + 1
                } fallido para la llamada al agente con datos de formulario:`,
                retryError.message
              );
          ...
          setMessages((prev) => {
            ...
            const tempMessageIndex = newMessages.findIndex(
              (msg) => msg.tempId === tempLoadingMessageId
            );
            if (tempMessageIndex !== -1) {
              ...
              if (agentError) {
                ...
                if (
                  agentError.message !== "AGENT_EXTERNAL_FAILED_AFTER_RETRIES"
                ) {
              ...
              newMessages.push({
                sender: "ai",
                content: mapAgentResponseToAIResponse(rawAgentResponse),
              });
            }
            return newMessages;
          });
        ...
        if (service === "gemini") {
          ...
          if (!chat)
            throw new Error(
              "Chat session not initialized for Gemini. This should be handled by useEffect."
            );
          ...
          throw new Error(
            `Ningún servicio de IA (Gemini/OpenAI) configurado para el modo seleccionado: '${service}'.`
          );
        }

        // Check if the selected LLM is configured (has an API key)
        if (
          (service === "gemini" && !geminiClient) ||
          (service === "openai" && !openaiClient)
        ) {
          throw new Error(
            `El servicio de IA (${service}) no está configurado con una clave API válida. Por favor, revisa la sección de configuración.`
          );
        }
        ...
        if (isLLMGemini) {
          ...
          responseFromLLM = await (chat as Chat).sendMessage({
            message: prompt,
          });
        } else {
          // OpenAI (activeLLMClient is openaiClient)
        ...
        let toolCalls =
          responseFromLLM.functionCalls || responseFromLLM.tool_calls || [];
        ...
        while (toolCalls && toolCalls.length > 0) {
          ...
          const toolExecutionPromises = toolCalls.map(async (call: any) => {
            ...
            if (
              agenteClient &&
              (functionName === "callExternalAgentWithQuery" ||
                functionName === "callExternalAgentWithData")
            ) {
              ...
              if (functionName === "callExternalAgentWithQuery") {
                ...
                if (typeof payload !== "string")
                  throw new Error(
                    "Parámetro 'query' inválido para callExternalAgentWithQuery."
                  );
              } else {
                ...
                if (typeof payload !== "object" || payload === null)
                  throw new Error(
                    "Parámetro 'data' inválido para callExternalAgentWithData."
                  );
              }
              ...
              for (let attempt = 0; attempt <= MAX_AGENT_RETRIES; attempt++) {
                try {
                  const rawAgentResponse = await agenteClient.sendToAgent(
                    payload,
                    agenteWebhookUrl
                  );
                  // Ensure the response is actually an object before returning
                  ...
                  console.error(
                    `Intento ${attempt + 1}/${
                      MAX_AGENT_RETRIES + 1
                    } fallido para la llamada al agente:`,
                    retryError.message
                  );
                  if (attempt < MAX_AGENT_RETRIES) {
              ...
              throw new Error(
                "Ruta de error inesperada para la llamada al agente."
              );
            } else if (!isAgentModeSelected) {
              // Direct Supabase tools, ONLY if NOT in agent mode
              // handleFunctionExecution returns { functionResponse: { id, name, response: { result: { ... } } } }
              return await handleFunctionExecution(
                { name: functionName, args: functionArgs, id: toolCallId },
                supabase
              );
            } else {
              throw new Error(
                `Función '${functionName}' no soportada o no implementada para el modo agente con ${
                  isLLMGemini ? "Gemini" : "OpenAI"
                } como orquestador.`
              );
            }
          });
          ...
          if (isLLMGemini) {
            ...
            const geminiFormattedToolResponses = toolCalls.map(
              (originalCall: any, i: number) => {
                ...
                if (
                  toolExecutionResult &&
                  toolExecutionResult.functionResponse
                ) {
            ...
            responseFromLLM = await (chat as Chat).sendMessage({
              message: geminiFormattedToolResponses,
            });
          } else {
            ...
            const newToolMessages = toolCalls.map((call: any, i: number) => {
              ...
              const contentString =
                typeof toolExecutionResult === "object" &&
                toolExecutionResult !== null
                  ? toolExecutionResult.functionResponse?.response?.result ||
          ...
          toolCalls =
            responseFromLLM.functionCalls || responseFromLLM.tool_calls || []; // Update for next iteration
        }

        // --- 4. Process Final Response ---
        const finalResponseContent: string =
          responseFromLLM.text || responseFromLLM.content;
        ...
        if (parsedAIResponse) {
          const aiMessage: Message = {
            ...
            tool_calls: responseFromLLM.tool_calls,
          };
          setMessages((prev) => [...prev, aiMessage]);
        } else {
          console.warn(
            "AI response was not valid JSON. Displaying as text. Response:",
        ...
        let statusDisplay: ConfirmationMessage = {
          icon: "error",
          title: "Error General",
          message: userFacingMessage,
        };
        ...
        if (error.message === "AGENT_EXTERNAL_FAILED_AFTER_RETRIES") {
          userFacingMessage =
            "Lo siento, el agente externo no pudo procesar tu solicitud después de varios intentos. Por favor, inténtalo de nuevo más tarde o reformula tu pregunta. Si el problema persiste, contacta a soporte técnico.";
          statusDisplay = {
            icon: "error",
          ...
          userFacingMessage =
            "Lo siento, hubo un problema al procesar la respuesta de la IA. Parece que el formato de los datos no es el esperado. Por favor, intenta reformular tu pregunta o notifica a soporte.";
          statusDisplay = {
            icon: "error",
            title: "Error de Formato Interno",
            message: userFacingMessage,
          };
        } else if (
          error.message.includes("client not initialized") ||
          error.message.includes("not configured with a valid API key")
        ) {
